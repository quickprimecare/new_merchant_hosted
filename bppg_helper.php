...
class BPPGModule
{
  ...
  /**
    ...
    /**
     * [$card_exp_dt description]
     * @var [type]
     */
    private $card_exp_dt;
     private $card_customer_name;
    /**
     * [$cvv description]
     * @var [type]
     */
    ...
    public function setPayment_Type($payment_type)
    {
       ...
       /**
     * @param string $mop_type
     *
     * @return self
     */
    public function setMopType($mop_type)
    {
    ...
    public function setCardNumber($card_number)
    {
        ...
        /**
     * @param string $card_exp_dt
     *
     * @return self 
     */
    public function setCardExpDate($card_exp_dt)
    {
 ...
 /**
     * @param string $card_customer_name
     *
     * @return self
     */
    public function setCardHolderName($card_customer_name)
    {
        $this->card_customer_name = $card_customer_name;
    ...
    public function generateEncryption($postdata,$hash)
    {
        ksort($postdata);
        unset($postdata["HASH"]);
        $all = '';
    ...
    public function createTransactionRequest()
    {
        ...
        $post_variables = array(
            ...
            "PAYMENT_TYPE" => $this->_empty($this->payment_type),
            // "MOP_TYPE" => $this->_empty($this->mop_type),
            "CUST_NAME" => $this->_empty($this->card_customer_name),
            "CARD_NUMBER" => $this->_empty($this->card_number),
            "CUST_EMAIL" => $this->_empty($this->cust_email),
            "CUST_PHONE" => $this->_empty($this->cust_phone),
            "CARD_EXP_DT" => $this->_empty($this->card_exp_dt),
            "CVV" => $this->_empty($this->cvv),
    ...
    public function createTransactionRequestNb()
    {
        $this->performChecks();
        $post_variables = array(
            "PAY_ID" => $this->_empty($this->pay_id),
    ...
    public function validateResponse($response)
    {
     ...
     //print_r($postdata);
      //aggrepay
      //$salt='60f32bcdcee741cc';
      //agwo
      //$salt='216cf42849d64ec3';

      //aggreypay
      $salt='8535b0d335e545d4';
    ...
    public function redirectForm($postdata)
    {
     ...
     public function hostedredirectForm($postdata,$url)
    {
        $output = '<form id="payForm" action="'.$url.'" method="post">';
        foreach ($postdata as $key => $value) {
            $output .= '<input type="hidden" name="' . $key . '" value="' . $value . '">' . "\n";
        }
       //echo "<textarea>".$output."</textarea>";exit;
    ...
    public function aes_encyption($hash_string){
        $CryptoKey= "B6B9B57E71DF7B52B17D191D36259E0B"; //encryption key 32 chars
